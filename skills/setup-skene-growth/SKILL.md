...
4. **Add skene-growth MCP server**: Create a file `~/.config/skene-growth/config.toml` with content `api_key = <API key>` (replace the placeholder with the key from tsep 3)
